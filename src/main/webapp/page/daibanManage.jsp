...
<script type="text/javascript">
	...
	function formatAction(val, row) {
		return "<a href='javascript:openFinishTaskTab("
				+ row.id
				+ ")'>办理任务</a>&nbsp;<a href='javascript:backTaskTab("
				+ row.id
				+ ")'>回退</a>&nbsp;<a target='_blank' href='${pageContext.request.contextPath}/task/showCurrentView.action?taskId="
				+ row.id + "'>查看当前流程图</a>"
	}

	function openFinishTaskTab(taskId) {
		$.post("${pageContext.request.contextPath}/task/redirectPage.action", {
			taskId : taskId
		}, function(result) {
	...
	//回退
	function backTaskTab(taskId) {
		$.post("${pageContext.request.contextPath}/task/backTaskTab.action", {taskId : taskId}, function(result) {
			//var result = eval('(' + result + ')');
			if (result.success) {
				$.messager.alert("系统系统","回退成功！");
				$("#dg").datagrid("reload");
			}else{
				$.messager.alert("系统系统","回退失败！");
				return;
			}
		}, "json");	
	}
