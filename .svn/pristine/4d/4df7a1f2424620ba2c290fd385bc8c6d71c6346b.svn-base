...
<script type="text/javascript">
	function formatAction(val,row){
		if(row.state=='未提交'){
			return "<a href='javascript:startApply("+row.id+")'>提交申请</a>";/* ,<a href='javascript:deleteApply("+row.id+")'>撤销</a> */
		...
		/* else{
			return "<a href='javascript:backApply("+row.processInstanceId+","+row.id+")'>撤销</a>";
		} */
	}
	...
	//撤销请假流程申请
	function backApply(processInstanceId,id){
		$.post("${pageContext.request.contextPath}/leave/backApply.action",{processInstanceId:processInstanceId,id:id},function(result){
			if(result.success){
				$.messager.alert("系统系统","撤销申请提交成功！");
				$("#dg").datagrid("reload");
			}else{
				$.messager.alert("系统系统","撤销申请提交失败，请联系管理员！");
			}
		},"json");
	}
	
	//删除请假流程申请
	function deleteApply(leaveId){
		$.post("${pageContext.request.contextPath}/leave/deleteApply.action",{leaveId:leaveId},function(result){
			if(result.success){
				$.messager.alert("系统系统","删除申请提交成功！");
				$("#dg").datagrid("reload");
			}else{
				$.messager.alert("系统系统","删除申请提交失败，请联系管理员！");
			}
		},"json");
	}
</script>
...
<table id="dg" title="请假管理" class="easyui-datagrid"
 ...
 <thead>
 	<tr>
 		...
 		<th field="processInstanceId" width="30"  align="center">流程实例Id</th><!-- hidden="true" -->
