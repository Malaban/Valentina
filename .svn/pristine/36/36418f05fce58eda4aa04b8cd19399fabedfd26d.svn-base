...
<script type="text/javascript">
	...
	function formatAction(val, row) {
		return "<a href='javascript:openListActionDialog("+ row.id+ ")'>流程执行过程</a>&nbsp;<a href='javascript:openListCommentDialog("+ row.id + ")'>历史批注</a>&nbsp;<a target='_blank' href='${pageContext.request.contextPath}/task/showHisCurrentView.do?taskId=" + row.id + "&endtime="+row.endTime+"'>查看当前流程图</a>"
	}
...
<body style="margin: 1px">
	<table id="dg" title="已办任务管理" class="easyui-datagrid" fitColumns="true"
		...
		<thead>
			<tr>
				...
				<th field="endTime" width="100" align="center" id="endTime">结束时间</th>
	...
	<div id="dlg3" class="easyui-dialog"
		...
		<table id="dg3" title="流程执行过程列表" class="easyui-datagrid"
			fitColumns="true"
			url="${pageContext.request.contextPath}/task/listAction.do"
